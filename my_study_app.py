...
def save_result_to_gsheet(quiz_title, user_id, score, duration, wrong_keywords):
    ...
    if res_ws:
        res_ws.append_row([
            quiz_title, user_id, score, round(duration, 2), 
...
with st.sidebar:
    ...
    # 🪄 [핵심] AI 출제 프롬프트 (비밀번호 입력 전에도 무조건 노출)
    st.info("🪄 **AI 출제 프롬프트 (지문 인식형)**")
    # 중괄호 에러 방지를 위해 f-string 미사용
...
[K] 핵심 키워드""", language="text")
    ...
    if pw_input == ADMIN_PASSWORD:
        ...
        # 설정 메뉴
        admin_settings['default_category'] = st.text_input("📌 처음 열릴 카테고리", value=admin_settings.get('default_category', ''), placeholder="예: 고1 국어")
        ...
        with st.expander("🆕 새 퀴즈 만들기", expanded=False):
            ...
            new_tx = st.text_area("AI 결과물 붙여넣기", height=150)
        ...
        with st.expander("🗑️ 퀴즈 삭제", expanded=False):
            ...
            for idx, q in enumerate(all_q):
                ...
                if c2.button("X", key=f"del_{idx}"):
                    if delete_quiz_from_gsheet(q.get('Title')):
                        st.rerun()
...
if quiz_data_list:
    ...
    if st.session_state.selected_quiz_title:
        curr_q = next((q for q in quiz_data_list if q['Title'] == st.session_state.selected_quiz_title), None)
        if curr_q:
            quiz_content = robust_parse(curr_q['Content'])
            ...
            # 퀴즈 진행 화면
                ...
                for idx, item in enumerate(quiz_content):
                    ...
                    # 📌 지문 인식 및 박스 렌더링
                    raw_q = item['q']
                    if "<지문>" in raw_q and "</지문>" in raw_q:
                        raw_q = raw_q.replace("<지문>", '<div class="passage-container">').replace("</지문>", '</div>')
                        st.markdown(raw_q, unsafe_allow_html=True)
                    else:
                        st.markdown(f'<div class="passage-container">{raw_q}</div>', unsafe_allow_html=True)
                    
                    # 보기 선택
                    is_answered = f"ans_{idx}" in st.session_state.user_answers
                    ans = st.radio(f"보기_{idx}", item['o'], index=None, key=f"r_{idx}", disabled=is_answered and not admin_settings['allow_change'], label_visibility="collapsed")
        ...
        if st.session_state.quiz_finished:
            st.balloons()
            st.success(f"🎉 퀴즈 종료! {st.session_state.player_name}님의 점수는 {int(st.session_state.last_score)}점입니다.")
