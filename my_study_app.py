...
with st.sidebar:
    st.header("⚙️ 관리자 설정")
    pw = st.text_input("관리자 암호", type="password", key="admin_pw_input")
    
    if pw == "1234":
        st.success("인증됨")
        
        # 📌 [핵심] AI 출제 프롬프트 가이드 (최상단 배치)
        st.info("🪄 **AI 출제 프롬프트 (복사해서 사용)**")
...
[K] 핵심 키워드"""
        ...
        if results:
            ...
            if ws_wrong:
                ...
                st.caption(f"📊 취약: {', '.join([f'{k}({v}회)' for k, v in counts.most_common(2)])}")
            ...
            if st.button("🏁 최종 제출하기", use_container_width=True):
                if len(st.session_state.user_answers) >= len(quiz_content):
                    ...
                    if res_ws:
                        ...
                        res_ws.append_row([st.session_state.selected_quiz_title, st.session_state.player_name, score, round(duration, 2), time.strftime('%Y-%m-%d %H:%M:%S')])
