...
for key in ['player_name', 'selected_quiz_title', 'user_answers', 'quiz_finished', 'start_time']:
    if key not in st.session_state:
        st.session_state[key] = {} if key == 'user_answers' else False if key == 'quiz_finished' else None if key == 'start_time' else ""

@st.cache_resource
def get_admin_settings():
    return {"feedback_mode": "⚡ 실시간 팩폭 (즉시 확인)", "default_category": ""}
...
with st.sidebar:
    ...
    if pw_input == ADMIN_PASSWORD:
        ...
        # 🪄 AI 프롬프트 가이드 (인증 시 무조건 출력)
        st.info("🪄 **AI 출제 프롬프트 (지문 포함)**")
        st.code(f"""국어 문제 10개를 출제해줘. 취약 주제({weak_points})를 반영해줘.
...
[K] 핵심 키워드""", language="text")
        ...
        with st.expander("🆕 새 퀴즈 배포"):
            c = st.text_input("카테고리"); t = st.text_input("제목"); tx = st.text_area("결과물 붙여넣기", height=150)
            if st.button("배포하기", use_container_width=True):
                ws = get_worksheet("Quizzes", ["Category", "Title", "Content", "CreatedAt"])
...
if quiz_list:
    ...
    def_cat = admin_settings.get('default_category', '').strip()
    ...
    if st.session_state.selected_quiz_title:
            ...
            if st.button("🏁 최종 제출"):
                if len(st.session_state.user_answers) >= len(quiz_content):
                    ...
                    if ws_res:
                        ...
                        ws_res.append_row([st.session_state.selected_quiz_title, st.session_state.player_name, score, 0, time.strftime('%Y-%m-%d %H:%M:%S')])
        ...
        if st.session_state.quiz_finished:
            st.balloons(); st.success("🎉 종료!"); st.button("다른 퀴즈", on_click=lambda: st.rerun())
