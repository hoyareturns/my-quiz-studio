...
def show_wrong_answer_review(current_player, all_quizzes):
    ...
    with st.form("wrong_review_form"):
        ...
        for idx, wr in enumerate(wrong_records):
            ...
            if q_data.get('p'): st.info(q_data['p'])
            st.markdown(f"**Q. {q_data['q']}**")
            
            if q_data['o'] == ["주관식"]:
                user_inputs[f"wr_{idx}"] = st.text_input("답 입력", key=f"wr_in_{idx}")
            else:
                user_inputs[f"wr_{idx}"] = st.radio("보기", q_data['o'], index=None, key=f"wr_in_{idx}", label_visibility="collapsed")
        
        if st.form_submit_button("오답 정복 시도", use_container_width=True):
            for idx, wr in enumerate(wrong_records):
                u_ans = user_inputs.get(f"wr_{idx}")
