...
@media (max-width: 768px) { [data-testid="stTabs"] [data-testid="column"] { flex: 1 1 calc(50% - 8px) !important; min-width: calc(45%) !important; } }
</style>
...
# 📌 [핵심] view_mode 변수를 정의한 후 사용하도록 순서 조정
    ...
    if st.session_state.selected_quiz:
        ...
        if st.session_state.start_time is None and not st.session_state.quiz_finished:
            ...
            for idx, it in enumerate(parsed):
                ...
                if ans and "실시간" in app_settings.get('feedback_mode','실시간'):
            ...
            if st.button("🏁 최종 제출", use_container_width=True):
                ...
                if not q_res or score > max([r['Score'] for r in q_res]):
                    save_chat("💻 시스템", f"🚨 [{st.session_state.player_name}]님이 '{q_item['Title']}'의 새로운 지배자 등극!")
        ...
        if st.session_state.quiz_finished:
            ...
            if st.button("목록으로 돌아가기", use_container_width=True): st.session_state.selected_quiz = ""; st.rerun()