...
def show_admin_sidebar(app_settings, get_kst_time):
    ...
    if pw == ADMIN_PASSWORD:
        ...
        st.caption("앱 기본 설정")
        default_view = st.selectbox("처음 열릴 탭", VIEW_OPTIONS, index=VIEW_OPTIONS.index(app_settings.get('default_view', VIEW_OPTIONS[0])))
        if default_view != app_settings.get('default_view'):
            save_setting("default_view", default_view); st.rerun()
        ...
        if default_cat != app_settings.get('default_category'):
            save_setting("default_category", default_cat); st.rerun()

        cust_cat = st.text_input("카테고리 목록 (쉼표 구분)", app_settings.get("custom_categories", ""))
        if st.button("카테고리 저장"):
            save_setting("custom_categories", cust_cat); st.rerun()

        feedback_mode = st.selectbox("피드백 모드", FEEDBACK_MODES, index=FEEDBACK_MODES.index(app_settings.get('feedback_mode', FEEDBACK_MODES[0])))
        if feedback_mode != app_settings.get('feedback_mode'):
            save_setting("feedback_mode", feedback_mode); st.rerun()
        ...
        with st.expander("퀴즈 수정/삭제"):
            if all_q:
                ...
                if st.button("퀴즈 삭제"):
                    if delete_quiz(sel_tit): st.rerun()